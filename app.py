...
listing_status = st.sidebar.selectbox(
    "Listing Status",["Active", "Pending", "Coming Soon", "Active Under Contract", "Closed", "Canceled", "Expired", "Hold", "Withdrawn"],
    index=0 # Defaults to Active so you get immediate results!
)
...
with col2:
    max_price = st.number_input("Max Price", min_value=0, value=10000000, step=50000, help="Set to 0 for no max.")
...
def fetch_mls_listings(key, market, status, limit_size, list_type, min_p, max_p, min_bd, min_ba):
    ...
    # Notice: NO geographic bounds. We are pulling the whole MLS!
    params = {
        "market": market,
        "status": status,
        "limit": limit_size,
    ...
    if not api_key:
        ...
        with st.spinner(f"Querying {market_id.upper()} for {listing_status} properties..."):
            st.session_state.listings, raw_count = fetch_mls_listings(
                api_key, market_id, listing_status, max_results, listing_type_filter, min_price, max_price, min_beds, min_baths
            )
...
if st.session_state.listings:
    ...
    m_col1.metric("Total Properties Mapped", f"{len(st.session_state.listings)}")
